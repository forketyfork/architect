...
5. **Use FirstFrameGuard for visibility transitions.** When a UI component moves to a visible state (modal opens, toast appears), call `markTransition()` and return `guard.wantsFrame()` from the component's `wantsFrame` method to bypass idle throttling. Guards belong to things that can be drawn; a component that owns per-session state must disarm guards for sessions the current view mode cannot render, or the loop presents at vsync rate until they become visible. (See ADR-012.)
...
- **Decision:** Each `SessionState` maintains a monotonic `render_epoch` counter that increments on terminal content changes. The renderer's `RenderCache` tracks the last presented epoch per session and only re-renders when epochs diverge. A session's dirty state is a request for a frame, not a presentation guarantee: `app/frame_schedule.zig` may defer output-only requests to a 30 FPS cadence, while input and other interactive demand renders immediately. The frame loop has no periodic re-render. `processOutput` advances the epoch only when ghostty-vt's terminal or screen dirty bits, dirty rows in the visible regions, viewport pin, cursor state, active screen, dynamic colors, or agent icon indicate a visible change. A persistent render-target texture stores a `ContentKey` for every state outside cell contents. A full refresh clears and redraws the texture when terminal/screen-wide flags, page dirtiness, viewport/layout/hover/color/dead-state changes, cache composition/render-mode changes, or baked wave overlays require it. Otherwise, the renderer clears and redraws only ghostty-dirty rows plus the previous and current cursor rows. Full refreshes clear all consumed ghostty dirty sources; partial refreshes clear only row dirty bits, preserving wide/page flags for the next full refresh.
- **Context:** Re-rendering all terminal cells every frame is expensive (glyph shaping, texture creation). Most frames in a multi-terminal grid have no changes in most sessions. Epoch comparison is O(1) per session and avoids deep content diffing. Ghostty's row/page dirty tracking supplies the safe rendering granularity: row-level updates such as spinners redraw only their strips, while scrolling and other cross-row changes remain full-cache operations. Separating the render request from presentation lets frequent terminal output share a bounded cadence without delaying interactive input.
- **Alternatives considered:**
  - *Ghostty row/page dirty bits with a `ContentKey`* -- selected: use ghostty's row dirty bits for strip-level cache redraws and its page dirty bit for full redraws, while the key routes all non-cell pixel changes to the full path.
...
- **Consequences:** Each C import site has an isolated translation failure boundary, target-specific headers are only created and imported when applicable, and `src/c.zig` remains the stable SDL symbol surface for the rest of the codebase. The executable continues to own the SDL3, SDL3_ttf, proc, and framework link configuration. On macOS, the default target names the host architecture explicitly, the active SDK's framework and system-library paths are passed to Zig, and `objc` is linked explicitly for framework stubs that re-export `/usr/lib/libobjc.A.dylib`. The C wrapper is compiled against the active SDK while Zig translates only the narrow declaration.
- **Date:** 2026-09
