...
| `ui/text_render.zig` | Emoji-aware single-line text rendering plus overflow handling for UI fields. Apple Color Emoji is a non-scalable bitmap font with one 160 px strike, so a string rendered through the attached fallback comes back ~160 px tall; this splits a line into emoji and text runs, scales the emoji runs to the line height and composes them (the UI equivalent of the per-glyph cell scaling in `font.zig`). `drawClippedTail` keeps an overflowing line inside its field, tail-aligned so the caret stays visible, with a fade over the leading edge. | `TextTex`, `LineFonts`, `makeTextTexture()`, `drawClippedTail()`, `EdgeFade` | `c` (SDL_ttf, surfaces), `geom` |
