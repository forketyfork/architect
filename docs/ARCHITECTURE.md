...
| `ui/text_edit.zig` | Backspace semantics shared by every single-line text input (worktree name, recent-folder/reader/story search, diff comments): Cmd clears, Alt deletes a word, plain deletes one UTF-8 codepoint. Pure length math — components own their buffers. | `DeleteScope`, `scopeFromMods()`, `backspace()`, `lastWordStart()`, `lastCharacterStart()`, `name_separators`, `path_separators`, `prose_separators` | `c` (modifier flags) |
