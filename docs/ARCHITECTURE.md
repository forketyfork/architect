...
- **Decision:** The terminal interaction component owns selection geometry and exposes a cached robot action pill after mouse release, with its top-left corner anchored to the selection's bottom-right corner. Clicking it queues the selected text to a centered modal; the modal can be cancelled or launched, and launching queues a typed action so the runtime creates the new session. Codex receives the composed prompt as its positional `PROMPT` launch argument; agents without that launch integration retain the process-detection-and-send path.
