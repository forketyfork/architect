...
- **Context:** To persist an agent's session ID for resumption on next launch, Architect must capture the session UUID that the agent prints to the PTY during graceful shutdown. The quit sequence is: detect running agent via macOS `sysctl`/process inspection → inject `Ctrl+C` twice (all supported agents) → block-read PTY output for a bounded window → if still foreground, retry `Ctrl+C` twice and drain again → if the agent remains foreground, send SIGTERM to the foreground process group as last resort and drain briefly again → extract the UUID with `terminal_history.extractLastUuid()` → persist it in `persistence.toml`. This flow runs entirely after the last rendered frame, outside the frame loop.
...
- **Layer boundary:** `session/state.zig` owns all PTY I/O (exit command injection, SIGTERM delivery, drain). `app/terminal_history.zig` owns text analysis (UUID extraction). `app/runtime.zig` orchestrates the two and owns persistence. This preserves the session-layer / app-layer boundary: session code never calls into app code.
- **Alternatives considered:**
