...
- **Decision:** The terminal interaction component owns selection geometry and exposes a cached robot action pill after mouse release, with its top-left corner anchored to the selection's bottom-right corner. Clicking it queues the selected text to a centered modal; the modal can be cancelled or launched, and launching queues a typed action so the runtime creates the new session and sends the composed prompt only after the selected agent process is detected.
...
- **Layout and rendering:** The modal shows a read-only preview of the selected context with Cancel and Launch actions. Prompt lines wrap to the field width, retain only the visible tail for large inputs, and reuse unchanged cached textures. Selection action geometry is hidden when scrolling or terminal layout changes invalidate its cell anchor.
