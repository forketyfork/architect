...
- **Decision:** The terminal interaction component owns selection geometry and exposes a cached action pill after mouse release. Clicking it queues the selected text to a centered modal; launching from that modal queues a typed action, and the runtime creates the new session and sends the composed prompt only after the selected agent process is detected.
...
- **Prompt format:** The runtime receives `<instructions>`, a blank line, `---`, the selected text, and a final `---`; the selected session's tracked cwd is passed to `SessionState.ensureSpawnedWithDir()` for the new terminal. Pending sends use stable session IDs and support multiple in-flight requests. Delivery prefers detecting the requested agent as the session's foreground process and sending once it's confirmed; if that isn't confirmed by a fixed deadline, the runtime sends the text anyway rather than dropping it, so a slow-starting agent still gets its context instead of losing it silently.
- **Layout and rendering:** The modal shows a read-only preview of the selected context. Prompt lines wrap to the field width, retain only the visible tail for large inputs, and reuse unchanged cached textures. Selection action geometry is hidden when scrolling or terminal layout changes invalidate its cell anchor.
