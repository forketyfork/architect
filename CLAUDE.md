...
- On macOS hosts where `/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk` is arm64e-only, the Nix dev shell and Homebrew formula retain a Zig 0.16.0 workaround: when `MacOSX15.4.sdk` is available, it points `DEVELOPER_DIR` at a fake developer dir backed by that SDK and installs an `xcrun` shim inside the fake developer tree so `zig build`, Ghostty's Apple SDK discovery, and unrelated tools that still invoke `/usr/bin/xcrun` keep working. If 15.4 is unavailable, the helper is a no-op; `build.zig` uses the active SDK's framework and system-library paths and explicitly links `objc` for current framework stubs. Keep the fallback until a macOS host confirms the upstream fix for https://codeberg.org/ziglang/zig/issues/31756 makes it unnecessary.
...
- First-frame guards belong to things that can be drawn; components that own per-session state must disarm guards for sessions the current view mode cannot render, or the loop presents at vsync rate until they become visible. `markDrawn()` goes at the top of `render`, before any visibility early return, because hide transitions arm the guard too. Every `wantsFrame` predicate must be backed by state that is guaranteed to reach a non-demanding value through `update()` alone, without depending on rendering or on user input.
...
- When a UI component moves into a new visible state (modals expanding, toasts appearing, gesture indicators starting), use `src/ui/first_frame_guard.zig`: call `markTransition()` when the state flips and `markDrawn()` at the top of every render; have `wantsFrame` return `guard.wantsFrame()` so the main loop renders immediately even under idle throttling.
