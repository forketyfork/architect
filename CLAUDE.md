...
- The notification and control socket listeners block in `poll(2)` on their listening socket plus a `WakePipe` self-pipe (`src/wake_pipe.zig`); shutdown stores the stop flag, signals the pipe, and then joins the thread so fixed-interval accept polling does not create background wakeups.
