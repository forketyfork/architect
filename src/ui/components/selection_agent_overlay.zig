...
pub const SelectionAgentOverlayComponent = struct {
    ...
    context_preview_tex: [context_preview_line_count]?TextTexture = [_]?TextTexture{null} ** context_preview_line_count,
    context_preview_count: usize = 0,
    context_preview_overflow_lines: usize = 0,
    context_preview_overflow_tex: ?TextTexture = null,
        ...
        switch (event.type) {
            ...
            c.SDL_EVENT_MOUSE_BUTTON_DOWN => {
                ...
                if (self.dropdown_open) {
                    ...
                    if (geom.containsPoint(dropdown, mouse_x, mouse_y)) {
                        const item_height = dpi.scale(dropdown_item_height, host.ui_scale);
                        const item_idx: usize = @intCast(@divFloor(mouse_y - dropdown.y, item_height));
                        if (item_idx < agent_count) self.selected_agent = item_idx;
                    }
                    self.dropdown_open = false;
                    self.prompt_focused = true;
                    return true;
    ...
    fn wantsFrame(self_ptr: *anyopaque, _: *const types.UiHost) bool {
        const self: *SelectionAgentOverlayComponent = @ptrCast(@alignCast(self_ptr));
        return self.visible and (self.guard.wantsFrame() or self.prompt_focused);
    }
    ...
    fn render(self_ptr: *anyopaque, host: *const types.UiHost, renderer: *c.SDL_Renderer, assets: *types.UiAssets) void {
        ...
        self.ensureContextPreview(renderer, host, cache) catch |err| {
    ...
    fn renderContextPreview(self: *SelectionAgentOverlayComponent, renderer: *c.SDL_Renderer, host: *const types.UiHost, rect: geom.Rect) void {
        ...
        for (self.context_preview_tex[0..self.context_preview_count], 0..) |texture, line_index| {
            if (texture) |line| {
                _ = c.SDL_RenderTexture(renderer, line.tex, null, &c.SDL_FRect{
                    .x = @floatFromInt(rect.x + inner),
                    .y = @floatFromInt(rect.y + inner + @as(c_int, @intCast(line_index)) * line_height),
                    .w = @floatFromInt(line.w),
                    .h = @floatFromInt(line.h),
                });
            }
        }
        if (self.context_preview_overflow_tex) |overflow| {
            _ = c.SDL_RenderTexture(renderer, overflow.tex, null, &c.SDL_FRect{
                .x = @floatFromInt(rect.x + inner),
                .y = @floatFromInt(rect.y + inner + @as(c_int, @intCast(self.context_preview_count)) * line_height),
                .w = @floatFromInt(overflow.w),
                .h = @floatFromInt(overflow.h),
            });
        }
        _ = c.SDL_SetRenderClipRect(renderer, if (had_clip) &previous_clip else null);
    }
    ...
    fn ensureContextPreview(self: *SelectionAgentOverlayComponent, renderer: *c.SDL_Renderer, host: *const types.UiHost, cache: *font_cache.FontCache) !void {
        const font_size = dpi.scale(14, host.ui_scale);
        if (self.context_preview_generation == cache.generation and self.context_preview_font_size == font_size) return;

        self.invalidateContextPreview();
        const selected_text = self.selected_text orelse return;
        const fonts = try cache.get(font_size);
        var lines = std.mem.splitScalar(u8, selected_text, '\n');
        while (self.context_preview_count < context_preview_line_count) {
            const line = lines.next() orelse break;
            if (line.len == 0) continue;
            self.context_preview_tex[self.context_preview_count] = text_render.makeTextTexture(
                self.allocator,
                renderer,
                .{ .text = fonts.regular, .emoji = fonts.emoji },
                line,
                ...
                log.warn("failed to render selected context preview line: {}", .{err});
                break :blk null;
            };
            self.context_preview_count += 1;
        }
        while (lines.next()) |line| {
            if (line.len == 0) continue;
            self.context_preview_overflow_lines += 1;
        }
        if (self.context_preview_overflow_lines > 0) {
            var buf: [64]u8 = undefined;
            const label = std.fmt.bufPrint(&buf, "+ {d} more line{s} not shown", .{
                self.context_preview_overflow_lines,
                if (self.context_preview_overflow_lines == 1) "" else "s",
            }) catch |err| blk: {
                log.warn("failed to format context preview overflow indicator: {}", .{err});
                break :blk "+ more lines not shown";
            };
            self.context_preview_overflow_tex = text_render.makeTextTexture(
                self.allocator,
                renderer,
                .{ .text = fonts.regular, .emoji = fonts.emoji },
                label,
                ...
                log.warn("failed to render context preview overflow indicator: {}", .{err});
                break :blk null;
            };
        }
    ...
    fn invalidateContextPreview(self: *SelectionAgentOverlayComponent) void {
        for (&self.context_preview_tex) |*texture| destroyTexture(texture);
        destroyTexture(&self.context_preview_overflow_tex);
        self.context_preview_count = 0;
        self.context_preview_overflow_lines = 0;
...
test "formatAgentPrompt places the selected terminal context between separators" {
