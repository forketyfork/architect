...
pub const Font = struct {
    ...
    pub fn renderClusterFill(
        ...
        const pad_x: f32 = @max(1.0, @as(f32, @floatFromInt(target_width)) * 0.08);
        const pad_y: f32 = @max(1.0, @as(f32, @floatFromInt(target_height)) * 0.08);
        const dest_rect = c.SDL_FRect{
            .x = @as(f32, @floatFromInt(x)) - pad_x * 0.5,
            .y = @as(f32, @floatFromInt(y)) - pad_y * 0.5,
            .w = @as(f32, @floatFromInt(target_width)) + pad_x,
            .h = @as(f32, @floatFromInt(target_height)) + pad_y,
        };
