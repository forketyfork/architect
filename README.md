...
- **Pull requests** (⌘P) — when the focused terminal is inside a GitHub-cloned repo, the ⌘P pill shows the current branch's PR number (e.g. `#123`), scaled to fit the pill; expand it to list open PRs (via `gh pr list`) with search filtering and check out a branch with `gh pr checkout`
