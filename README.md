...
- **Interactive Control**:
  ...
  - Press ⌘1–⌘9/⌘0 to switch to terminals 1–10 directly (expands in grid view, switches in full view); hotkey badges are shown in grid cells
  ...
  - Open the git worktree picker with the ⌘T pill (left of ?) or ⌘T; it refreshes each time you open it, hides if the focused shell has a running process, lists up to 9 worktrees discovered from git metadata, shows paths relative to the primary worktree, highlights rows on hover with a soft gradient, lets you click or hit ⌘1–⌘9 to cd the focused shell into that worktree (no respawn), shows a delete (×) button for non-root worktrees, and offers ⌘0 to create a new worktree (prompts for a name, creates `.architect/<name>` via `git worktree add -b <name>`, then cds into it)
- **Keyboard Navigation**: Move the grid focus with ⌘↑/↓/←/→, switch to a specific terminal with ⌘1–⌘9/⌘0, and open the on-screen shortcut overlay via the ⌘/ hotkey (pill labeled ⌘?) in the top-right corner
- **Scrollback in Place**: Hover any terminal and use the mouse wheel or a trackpad to scroll history; small high-precision gestures accumulate via SDL's integer wheel deltas, typing snaps back to live output, and a yellow strip in grid view shows when you're scrolled (10 MB per terminal, matching Ghostty's default). When a terminal enables mouse tracking, scroll events are forwarded to the app while you stay in live output; outside the cell grid, scrollback still works.
